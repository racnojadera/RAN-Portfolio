...
//Prints the outputs.
void printLines(int indexCount, int digit1, int digit2, string reply)
{
...
void readToPrintLines(int indexCount)
{
    int agentFirstDigit, agentSecondDigit;
    string userSays;
    scanf("%d %d", &agentFirstDigit, &agentSecondDigit);
    getline(cin, userSays);
    printLines(indexCount, agentFirstDigit, agentSecondDigit, userSays);
}
