...
bool isTheTriangleGoingCCW(Point currentPoint, Point previousPoint, Point nextPoint)
{
	Vector2 previousToCurrent(currentPoint.x - previousPoint.x, currentPoint.y - previousPoint.y);
	Vector2 currentToNext(nextPoint.x - currentPoint.x, nextPoint.y - currentPoint.y);

	float crossProduct = getCrossProduct(previousToCurrent, currentToNext);
...
bool checkForPointsInTriangle(Triangle triangle, std::vector<Point> listOfPoints)
{
	for (int i = 0; i < listOfPoints.size(); i++)
	{
		if (!(listOfPoints[i] == triangle.a) && (!(listOfPoints[i] == triangle.b)) && (!(listOfPoints[i] == triangle.c)))
		{
			Vector2 AtoP(listOfPoints[i].x - triangle.a.x, listOfPoints[i].y - triangle.a.y);
			Vector2 AtoB(triangle.b.x - triangle.a.x, triangle.b.y - triangle.a.y);

			Vector2 BtoP(listOfPoints[i].x - triangle.b.x, listOfPoints[i].y - triangle.b.y);
			Vector2 BtoC(triangle.c.x - triangle.b.x, triangle.c.y - triangle.b.y);

			Vector2 CtoP(listOfPoints[i].x - triangle.c.x, listOfPoints[i].y - triangle.c.y);
			Vector2 CtoA(triangle.a.x - triangle.c.x, triangle.a.y - triangle.c.y);

			float BAxPA = getCrossProduct(AtoB, AtoP);
			float CBxPB = getCrossProduct(BtoC, BtoP);
			float ACxPC = getCrossProduct(CtoA, CtoP);

			bool isThereANegativeProduct = (BAxPA < 0) || (CBxPB < 0) || (ACxPC < 0);
			bool isThereAPositiveProduct = (BAxPA > 0) || (CBxPB > 0) || (ACxPC > 0);

			if (!(isThereANegativeProduct && isThereAPositiveProduct))
			{
				return true;
			}
		}
...
std::vector<Triangle> Triangulate(const Polygon& polygon) {
	...
	std::vector<Point> outlinePoints = polygon.outline;

	int i = 0;
	while (outlinePoints.size() > 3)
	{
		bool triangleFollowingCCW; //All test inputs are arranged in CCW order
		
		if (i == 0)
		{
			triangleFollowingCCW = isTheTriangleGoingCCW(outlinePoints[0], outlinePoints[outlinePoints.size() - 1], outlinePoints[1]);
		}
		else
		{
			triangleFollowingCCW = isTheTriangleGoingCCW(outlinePoints[i], outlinePoints[i - 1], outlinePoints[i + 1]);
		}

		if (triangleFollowingCCW)
		{
			Triangle newPotentialTriangle;
			bool areTherePointsInsideTriangle;

			if (i == 0)
			{
				newPotentialTriangle.a = outlinePoints[outlinePoints.size() - 1];
			}
			else
			{
				newPotentialTriangle.a = outlinePoints[i - 1];
			}
			newPotentialTriangle.b = outlinePoints[i];
			newPotentialTriangle.c = outlinePoints[i + 1];

			areTherePointsInsideTriangle = checkForPointsInTriangle(newPotentialTriangle, outlinePoints);

			if (!areTherePointsInsideTriangle)
			{
				ret.push_back(newPotentialTriangle);
				outlinePoints.erase(outlinePoints.begin()+i);
			}
			else
			{
				if (i == outlinePoints.size() - 1)
				{
					i = 0;
				}
				else
